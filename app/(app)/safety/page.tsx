  ...
  return (
    <>
      ...
      <Section title="🩸 ברזל — הדבר הכי חשוב תזונתית">
        <p style={p}>מאגרי הברזל מהלידה מתדלדלים סביב גיל 6 חודשים. התכנית כוללת את מקורות הברזל החשובים: <b>חלמון ביצה והודו (שבוע 2), עדשים (שבוע 3), עוף (שבוע 4), בשר בקר (שבוע 5), טחינה (שבוע 6)</b>. טיפ: ויטמין C (עגבניה, פלפל, תפוז) משפר ספיגת ברזל — שווה לשלב באותה ארוחה. אחרי ההיכרות, כדאי להגיש מזון עשיר בברזל כמעט כל יום.</p>
      </Section>

      <Section title="💊 תוספים — הנחיות משרד הבריאות">
        <ul style={ul}>
          <li><b>ויטמין D</b> — 400 יחב״ל (טיפות) מדי יום מהלידה ועד גיל שנה, לכל התינוקות</li>
          <li><b>ברזל (טיפות)</b> — מגיל 4 חודשים עד שנה וחצי, במינון לפי הנחיית טיפת חלב/רופא. גם כשהתזונה כוללת מזונות עשירים בברזל</li>
          <li>נותנים את טיפות הברזל בין הארוחות או עם מזון עשיר בוויטמין C — לא יחד עם מוצרי חלב (החלב מפריע לספיגה)</li>
        </ul>
      </Section>
      ...
      <Section title="🚨 כללי הזהב לאלרגנים">
        <ul style={ul}>
          ...
          <li><b>פרוטוקול ההכנסה:</b> מנה 1 (רבע כפית) ← למחרת מנה 2 (חצי כפית) — יומיים רצופים! ← מנה 3 (כפית) בסוף השבוע</li>
